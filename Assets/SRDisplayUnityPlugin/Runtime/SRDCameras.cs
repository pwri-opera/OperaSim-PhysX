...
namespace SRD.Core
{
    ...
    public class SRDCameras
    {
        ...
        private GameObject GetOrCreateEyeCameraObj(EyeType type)
        {
            ...
            GameObject eyeCameraObj = null;
            foreach (Transform child in eyeAnchorObj.transform)
            {
                if (child.name.Contains(eyeCameraName))
                {
                    eyeCameraObj = child.gameObject;
                }
            }
            ...
            eyeCameraObj.name = eyeCameraName + "_" + _srdManager.DeviceIndex;

            SRDSceneEnvironment.InitializePose(eyeCameraObj.transform);
            return eyeCameraObj;
        }

        internal void ToggleWatcherCamera(bool isActive)
        {
        ...
        {
            positionInScreen = SRDApplicationWindow.GetSRDMousePos(positionInScreen);
            return SRDScreenToWorldPoint(positionInScreen);
        }

        internal Vector3 SRDScreenToWorldPoint(Vector2 positionInSRDScreen)
        {
            var screenRate = new Vector2(positionInSRDScreen.x / SRDApplicationWindow.SRDScreenRect.width, positionInSRDScreen.y / SRDApplicationWindow.SRDScreenRect.height);
        ...
        {
            if(_srdManager.IsLensShiftEnabled && Screen.width == SRDApplicationWindow.SRDScreenRect.width && Screen.height == SRDApplicationWindow.SRDScreenRect.height)
            {
            ...
            var h = SRDHelper.CalcHomographyMatrix(_srdManager.DisplayEdges.LeftUp.position,
                                                   ...
                                                   _watcherCamera);
            var x = positionInScreen.x / SRDApplicationWindow.SRDScreenRect.width;
            var y = positionInScreen.y / SRDApplicationWindow.SRDScreenRect.height;
        ...
        public Vector2 SRDScreenToScreen(Vector2 positionInSRDScreen)
        {
            ...
            var w = h[6] * x + h[7] * y + h[8];
            return new Vector2(SRDApplicationWindow.SRDScreenRect.width * (h[0] * x + h[1] * y + h[2]) / w,
                               SRDApplicationWindow.SRDScreenRect.height * (h[3] * x + h[4] * y + h[5]) / w);
        }

        internal Camera GetEyeCamera(EyeType type)
        {
            if(!_eyeCameras.ContainsKey(type))
            {
                var eyeCameraObj = GetOrCreateEyeCameraObj(type);
